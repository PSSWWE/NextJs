...
export default function VendorTransactionsPage() {
  ...
  const fetchVendorData = async (options?: { recalc?: boolean }) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        console.log(`[Vendor Transactions] Data loaded in ${duration.toFixed(2)}ms (${(duration / 1000).toFixed(2)}s)`);
      } else {
                  ...
                  <View style={{ border: '1 solid #ddd', padding: 15, backgroundColor: '#fafafa', marginBottom: 15 }}>
                    <View style={{ flexDirection: 'row', flexWrap: 'wrap', gap: 15 }}>
                      <View style={{ width: '48%' }}>
                        ...
                        <Text style={{ fontSize: 11, color: '#333', fontWeight: 'bold' }}>{vendor.CompanyName || 'Vendor Name'}</Text>
                      </View>
                      <View style={{ width: '48%' }}>
                        <Text style={{ fontSize: 9, color: '#666', textTransform: 'uppercase', marginBottom: 3, fontWeight: 'bold' }}>Account Number</Text>
                        <Text style={{ fontSize: 11, color: '#333' }}>{vendor.id || 'N/A'}</Text>
                      </View>
                      <View style={{ width: '100%', marginTop: 5 }}>
                        ...
                        <Text style={{ fontSize: 10, color: '#333', lineHeight: 1.4 }}>
                          {(vendor as any).Address || ''}{(vendor as any).Address && ((vendor as any).City || (vendor as any).Country) ? ', ' : ''}
                          {(vendor as any).City || ''}{(vendor as any).City && (vendor as any).Country ? ', ' : ''}{(vendor as any).Country || ''}
                          {vendor.PersonName ? ` | Attn: ${vendor.PersonName}` : ''}
                        </Text>
                      </View>
                      <View style={{ width: '48%' }}>
